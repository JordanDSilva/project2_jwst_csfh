...
load_with_agn = foreach(i = 1:dim(galaxies)[1], .combine = bind_rows)%do%{
  print(i)
  foo = readRDS(paste0(proj_dir, "/ProSpectOut/withAGN/gal_", i, "_", galaxies$VID[i], "_", galaxies$MODULE[i], ".rds"))
  ...
  legend(x='topright', legend = paste0("z=",galaxies$z[i]))
...
load_without_agn = foreach(i = 1:dim(galaxies)[1], .combine = bind_rows)%dopar%{
  print(i)
  foo = readRDS(paste0(proj_dir, "/ProSpectOut/withoutAGN/gal_", i, "_", galaxies$VID[i], "_", galaxies$MODULE[i], ".rds"))
  testHigh = foo$highlander
  startpoint = foo$bestfit$parm
  ...
  plot(bestfit, ylim = c(10^-10, 10^-5))
  legend(x = "topright", legend = paste0("Redshift=", Redshift))
  plot(bestfit$SEDout, ylim = c(10^3, 10^10))
  dev.off()

  
  pdf(paste0(proj_dir, "/plots/ProSpect/withoutAGN/gal_", i, "_", galaxies$VID[i], "_", galaxies$MODULE[i], ".fits"),
      width = 10, height = 10)
  plot(foo$bestfit, xlim = 10^c(3.0, 5.0), ylim = 10^c(-10, -3))
  plot(foo$bestfit$SEDout, xlim = 10^c(3.0, 5.0), ylim = 10^c(3, 9))
  par(mfrow = c(1,1), mar = rep(1.0,4), oma = rep(1.0,4))
